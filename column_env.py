...
class RandomObstaclesEnv(gym.Env):
    def __init__(self, obstacle_x_count_range=(1,5), space_size=(30, 30), obs_dist=5, x_step_range=(3,5),y_step_range=(5,10),half_obstacle_size_range=(1,2),obs_format = 'array'):
    ...
    def reset(self): 
        ...
        obstacle_x_count=np.random.randint(self.obstacle_x_count_range[0],self.obstacle_x_count_range[1]) #the number verticle lines of columns
        ...
        if self.obs_format == 'array':
            ...
            self.state = {'position':position, 'map':self.map[0:3]}
    ...
    def update_inspected_column(self, position):
        ...
        for up_surface in self.surface_list[0]:            
            ...
            if y-self.obs_dist<position[1]<y and xmin<position[0]<xmax and already_inspected==False:
                inspected_column_map[y,xmin:xmax]=True
        ...
        for left_surface in self.surface_list[2]:
            ...
            if x-self.obs_dist<position[0]<x and ymin<position[1]<ymax:
                inspected_column_map[ymin:ymax,x]=True
            else:
    ...
    def step(self, action):
        self.step_count+=1 
        if self.obs_format == 'array':
            position = np.array([self.map[3,0,1],self.map[3,0,0]])
        elif self.obs_format == 'dict':
            position=self.state['position']
        penalty = 0  
        #take left top point as original point, x-axis is horizontal, y-axis is vertical
        if action == self.action_dict['up'] and position[1]!=0 : #'up'
            delta_position=np.array([0,-1])
        elif action == self.action_dict['down'] and position[1]!= self.y_length-1:#'down'
            delta_position=np.array([0,1])
        elif action == self.action_dict['left'] and position[0]!=0:#'left': 
            delta_position=np.array([-1,0])
        elif action == self.action_dict['right'] and position[0]!= self.x_length-1: #'right'
            delta_position=np.array([1,0])
        ...
        new_position=(position+delta_position).astype(int)
        ...
        self.map[3,0,0]=new_position[1]
        self.map[3,0,1]=new_position[0]
        if self.obs_format == 'array':
            self.state = self.map.copy()#torch.tensor(self.map.copy())
        elif self.obs_format == 'dict':
    ...
    def build_env(self):
        ...
        for obstacle in self.obstacles:
            #take left top point as original point, x-axis is horizontal, y-axis is vertical
            x, y = obstacle
            image[y-self.half_obstacle_size:y+self.half_obstacle_size, x-self.half_obstacle_size:x+self.half_obstacle_size] = False  # Color obstacles in red
